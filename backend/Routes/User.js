    ...
    if(user.length>0){
      bcrypt.compare(password,user[0].password,(err, result)=>{
        if(result){
          ...
          return res.status(200).send({"msg":"login successful","token":token,"details":{email,id:user[0]._id}});
      ...
      return res.status(400).send("Please fill correct email id")
    }
  } catch (error) {
    return res.status(400).send(error)
  }
...
UserRouter.patch("/follow/:id",async(req, res)=>{
  ...
  return res.status(200).send({msg:'follwing the people',users:newUser})
...
UserRouter.patch("/clear/:id",async(req, res)=>{
  ...
  const user = await UserModel.findByIdAndUpdate({_id:id},{friends:[]})  ;
    return res.send("cleared friend list")
  } catch (error) {
    console.log(error);
    return res.send(error)
  }
...
UserRouter.patch("/:id",async(req, res)=>{
  ...
  try {
    ...
    let newUser = await UserModel.findById(id)
    return res.status(201).send({msg:'user has been updated',user:newUser})
  } catch (error) {
    console.log(error);
    return res.send(error)
  }
...
UserRouter.get("/unfollowed/:id",async(req, res)=>{
  ...
  return res.status(200).send({unfollowedPeople})
  } catch (error) {
    console.log(error);
...
UserRouter.patch("/request/:id",async(req, res)=>{
  ...
  try {
    ...
    return res.status(201).send({msg:"friend request has been sent",user:updatedUser})
  } catch (error) {
    console.log(error);
    return res.status(404).send(error)
  }
...
UserRouter.patch("/unrequest/:senderId",async(req, res)=>{
  ...
  try {
    ...
    return res.status(200).send("friend request has been unsent")
...
UserRouter.patch("/accept/:id",async(req, res)=>{
  ...
  try {
    ...
    return res.status(201).send(`${followId} has been approved`)
  } catch (error) {
    console.log(error);
    return res.status(404).send(error)
  }
...
UserRouter.patch("/reject/:id",async(req, res)=>{
  ...
  try {
    ...
    return res.status(201).send(`${followId} rejected successfully`)
  } catch (error) {
    console.log(error);
    return res.status(404).send(error)
  }
